#include "plugin.hpp"
#include "expanded/ImpromptuModular.hpp"
	...
	void process(const ProcessArgs& args) override {
		if (Trigger1.process(params[B1_PARAM].getValue())) {
			...
			if (expandablePresent) {
				DEBUG("Yes module!");
				if (this->leftExpander.module->model == modelPhraseSeq16) {
					DEBUG("It's a modelPhraseSeq16!!");
				} else {
					DEBUG("Nope, it's a %s", this->leftExpander.module->model->getFullName().c_str());
				}
				if (!isPs16) {
					ps16 = dynamic_cast<PhraseSeq16*>(this->leftExpander.module);
					if (ps16) {
						DEBUG("But then, yes!");
						isPs16 = true;
					}
		...
		if (Trigger2.process(params[B2_PARAM].getValue())) {
			if (isPs16) {
				...
				DEBUG("Step index is %d", ps16->stepIndexEdit);
				 // = seqIndexEdit;
			}
		}
	}
